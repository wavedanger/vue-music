<template>
  <div class="player" v-show="playlist.length>0">
    ...
    <play-list ref="playlist" @refreshScroll="refreshScroll2"></play-list>
    <audio ref="audio" :src="currentSong.url"  @play="ready"  @error="error" @timeupdate="updateTime" @ended="end"></audio>
     <top-tip ref="topTip" :delay="delay">
        <div class="tip-title">
          <i class="icon-close"></i>
          <span class="text">歌曲获取失败,自动播放下一首</span>
        </div>
      </top-tip>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    refreshScroll2() {
      console.warn('歌词可能无法滚动，请稍等！')
      clearTimeout(this.timer)
      this.timer = setTimeout(() => {
        this.$refs.lyricList.refresh()
        console.info('歌词开以滚动啦！')
      }, 1000)
    },
    showTip() {
      this.delay = 1000
  ...
  watch: {
    currentSong(newSong, oldSong) {
      ...
      // })
      clearTimeout(this.timer)
      this.timer = setTimeout(() => { // 确保DOM已存在
        this.$refs.audio.play()
        this.getLyric()
      }, 1000)
    },
